...
class HandoverInjectDetector(BaseDetector):
    ...
    def _fmt(self, events: List[Dict]) -> List[str]:
        ...
        for e in events[:6]:
            ...
            extras = ""
            if t304:  extras += f" T304={t304}00ms"
            if rnti:  extras += f" C-RNTI={rnti}"
            lines.append(
                f"[{ts}] {msg} targetPCI={tpci} EARFCN={tearfcn}"
                f"{extras} {mci} {prose} ({src})"
            )
        return lines

    def _target_summary(self, events: List[Dict]) -> str:
        """Summarise unique target cells across all injected handovers."""
        targets = {}
        for e in events:
            pci = e.get("target_pci")
            earfcn = e.get("target_earfcn")
            if pci and earfcn:
                key = (pci, earfcn)
