...
### Fixed
- OpenCelliD integration fully operational. `config.yaml` `enabled` key now correctly parsed. API key `REDACTED_API_KEY` active. All runs now show `OpenCelliD: enabled` in header.
