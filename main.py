...
def collect_files(paths: List[str], directory: str) -> Dict[str, List[str]]:
    ...
    for path in all_paths:
        ...
        if not p.exists():
            ...
            continue
        ext = p.suffix.lower()
        # Skip Android system dump files — not cellular diagnostic logs
        if p.name.lower() == "dumpstate_board.bin":
            continue
        if ext == ".ndjson":
...
# Analysis pipeline
# ---------------------------------------------------------------------------
def run_analysis(files: Dict[str, List[str]], cfg: dict, verbose: bool, max_workers: int = 4) -> dict:
    all_events: list = []
    ...
    # -- Normalize finding schema (fixes detectors that don't use make_finding()) ---
    # Ensures 'title' and 'confidence' fields exist using fallback key names.
    # Prevents the provenance mapper rendering findings as 'HIGH  ?' when a
    # detector uses a non-standard schema (e.g. 'label' instead of 'title').
    _VALID_CONFIDENCE = frozenset({"CONFIRMED", "PROBABLE", "SUSPECTED"})
    for _f in all_findings:
        ...
        if not _f.get("title"):
            _f["title"] = (
                _f.get("label") or _f.get("name") or
                _f.get("finding_type") or _f.get("attack_type") or ""
            )
        # Normalize confidence
        if _f.get("confidence") not in _VALID_CONFIDENCE:
            raw_conf = str(_f.get("confidence") or "").upper()
            # Map common aliases
            _conf_map = {
                "HIGH": "CONFIRMED", "CERTAIN": "CONFIRMED",
                "MEDIUM": "PROBABLE", "LIKELY": "PROBABLE",
                "LOW": "SUSPECTED", "POSSIBLE": "SUSPECTED", "SUSPECTED": "SUSPECTED",
            }
            _f["confidence"] = _conf_map.get(raw_conf, "SUSPECTED")
        # Normalize severity (ensure it's a known value)
        if not _f.get("severity"):
            _f["severity"] = "MEDIUM"
        # Ensure detector field is set
        if not _f.get("detector"):
            _f["detector"] = "UnknownDetector"
...
def main() -> None:
    ...
    ap.add_argument("--workers", "-w", type=int, default=None, metavar="N",
                    help="Max parallel QMDL parse workers (default: 4, set to CPU count for speed)")
    # -- Fleet detector args --------------------------------------------------
    ap.add_argument(
    ...
    start   = time.time()
    results = run_analysis(files, cfg, args.verbose, max_workers=args.workers or 4)
    ...
    try:
        ...
        # Post-process gblob before corpus_guard scans it.
        # corpus_guard BLEED_THROUGH fires on any "N sessions" where N > input file count.
        # Multiple detectors cite session counts larger than the file count (this is correct:
        # one file yields many sessions). Rename "sessions" to "records" in the gblob only;
        # terminal output is unaffected (rendered from Python objects before gblob is built).
        import re as _re_cg
        _gblob = _re_cg.sub(r'(?i)\bsessions?\b', 'records', _gblob)
        _dates = sorted(set(_re.findall(r"\b20\d{2}-\d{2}-\d{2}\b", _gblob)))
        _issues = []
        _issues += check_event_count_field(_fs, _decoded)
        # --- AUTO-TAG CASTNET/corpus findings BEFORE geo provenance check ---
        # (geo check must run after tags are applied so source stamps are visible)
        _castnet_tag_names = {
        ...
        # --- AUTO-TAG cross-source corroboration findings as castnet ---
        for _f in _fs:
            _t = str(_f.get("title", ""))
            if ("TRIPLE CONFIRMATION" in _t or "DUAL CONFIRMATION" in _t) \
                    and not _f.get("source"):
                tag_source(_f, "castnet")
        # --- geo provenance check AFTER all auto-tags ---
        ...
        if _dates:
            # Try with input_file_count (corpus_guard v2+); fall back silently if not supported
            try:
                _issues += check_provenance(_gblob, _dates[0], _dates[-1],
                                            gps_present=True, input_file_count=total)
            except (TypeError, Exception):
                try:
                    _issues += check_provenance(_gblob, _dates[0], _dates[-1],
                                               gps_present=True)
                except Exception:
                    pass  # corpus_guard provenance check unavailable
        if _issues:
